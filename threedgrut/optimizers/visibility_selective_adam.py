    ...
    def step(
        ...
        *,
        ...
        if advance_age:
            ...
            if bool(position_group["wrap_age"]):
                updated_steps = torch.remainder(
                    advanced_steps,
                    VISIBILITY_SELECTIVE_COUNTER_MAX + 1,
                )
            else:
                updated_steps = torch.clamp(
                    advanced_steps,
                    max=VISIBILITY_SELECTIVE_COUNTER_MAX,
                )
            gaussian_steps.index_copy_(0, visible_indices, updated_steps)
