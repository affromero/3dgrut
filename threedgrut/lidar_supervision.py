...
_PACKET_SCHEDULE_FINITE_SHUFFLE = "finite_shuffle"
_PACKET_SCHEDULE_PCG_QUEUE = "pcg_queue"
...
class LidarRaySampler:
    ...
    def __init__(
        ...
        seed: int,
        packet_schedule: str = _PACKET_SCHEDULE_PCG_QUEUE,
    ) -> None:
        if packet_schedule not in {
            _PACKET_SCHEDULE_FINITE_SHUFFLE,
            _PACKET_SCHEDULE_PCG_QUEUE,
        }:
            raise ValueError(
                "LiDAR packet_schedule must be 'finite_shuffle' or "
                f"'pcg_queue', got {packet_schedule!r}."
            )
        self._packet_schedule = packet_schedule
        for label, path in (
            ("map", map_path),
            ("poses", poses_path),
        ...
        if self._packet_schedule == _PACKET_SCHEDULE_FINITE_SHUFFLE:
            self._packet_order = np.arange(len(pose_data), dtype=np.int64)
            self._rng.shuffle(self._packet_order)
    ...
    def packet_count(self) -> int:
        """Return the number of packets in the configured source stream."""
        if self._packet_schedule == _PACKET_SCHEDULE_FINITE_SHUFFLE:
            return len(self._packet_order)
        return len(self._packet_queue)

    def mark_exhausted(self) -> None:
        """Move a finite source stream to its terminal state."""
        if self._packet_schedule == _PACKET_SCHEDULE_FINITE_SHUFFLE:
            self._packet_cursor = len(self._packet_order)

    def state_dict(self) -> dict[str, object]:
        """Return the packet source schedule and its replay state."""
        ...
        if self._packet_schedule == _PACKET_SCHEDULE_FINITE_SHUFFLE:
            state["packet_cursor"] = self._packet_cursor
            state["packet_order"] = self._packet_order.copy()
            return state
        state["packet_queue"] = list(self._packet_queue)
        state["pcg_state"] = self._pcg_state
        return state

    def load_state_dict(self, state: dict[str, object]) -> None:
        """Restore the configured packet schedule and its exact position."""
        packet_count = int(state["packet_count"])
        saved_schedule = str(state.get("packet_schedule", ""))
        if not saved_schedule:
            saved_schedule = (
                _PACKET_SCHEDULE_FINITE_SHUFFLE
                if "packet_order" in state
                else _PACKET_SCHEDULE_PCG_QUEUE
            )
        if saved_schedule != self._packet_schedule:
            raise ValueError(
                "LiDAR checkpoint packet schedule does not match the "
                "configured source."
            )
        ...
        if self._packet_schedule == _PACKET_SCHEDULE_FINITE_SHUFFLE:
            ...
            source_fingerprint = str(state.get("source_fingerprint", ""))
            ...
            if not isinstance(rng_state, dict):
                raise TypeError(
                    "LiDAR checkpoint RNG state must be a mapping."
                )
            packet_cursor = int(state.get("packet_cursor", -1))
            packet_order = np.asarray(state.get("packet_order"), dtype=np.int64)
            if not 0 <= packet_cursor <= packet_count:
                raise ValueError("LiDAR checkpoint packet cursor is invalid.")
            if packet_order.shape != (packet_count,) or not np.array_equal(
                np.sort(packet_order), np.arange(packet_count)
            ):
                raise ValueError("LiDAR checkpoint packet order is invalid.")
            self._packet_cursor = packet_cursor
            self._packet_order = packet_order.copy()
    ...
    def _next_packet_index(self) -> int | None:
        if self._packet_schedule == _PACKET_SCHEDULE_FINITE_SHUFFLE:
            if self._packet_cursor >= len(self._packet_order):
                return None
            packet_index = int(self._packet_order[self._packet_cursor])
    ...
    def _packet_points(self, packet_index: int) -> np.ndarray:
        ...
        if self._packet_schedule == _PACKET_SCHEDULE_FINITE_SHUFFLE:
            native_indices = start + 2 * np.arange(
                (end - start) // 2,
                dtype=np.int64,
            )
        else:
    ...
    def sample(
        ...
        *,
        ...
        if self._packet_schedule == _PACKET_SCHEDULE_FINITE_SHUFFLE:
            target_index = int(
                self._rng.integers(0, len(points), endpoint=False)
            )
        else:
