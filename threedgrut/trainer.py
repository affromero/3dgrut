...
class Trainer3DGRUT:
    ...
    def _radiance_gradient_scale(self) -> float:
        strategy_conf = self.conf.get("strategy")
        if strategy_conf is not None and bool(
            strategy_conf.get("reference_replay", False)
        ):
    ...
    def get_losses(
        ...
        fixed_image_loss_scale_by_resolution = bool(
            self.conf.loss.get("fixed_image_loss_scale_by_resolution", True)
        )
        ...
        if mask is not None:
            ...
            if use_fixed_image_loss_denominator:
                loss_denominator = fixed_image_loss_denominator(
                    ...
                    scale_by_image_resolution=(
                        fixed_image_loss_scale_by_resolution
                    ),
                    min_valid_fraction=(
            ...
            loss_denominator = fixed_image_loss_denominator(
                ...
                mask=None,
                image_scale=native_image_scale,
                scale_by_image_resolution=fixed_image_loss_scale_by_resolution,
    ...
    def run_lidar_step(
        ...
        _step_gaussian_optimizer(
            ...
            visibility=optimizer_visibility,
            advance_age=bool(
                lidar_conf.get("advance_gaussian_age", True)
            ),
        )
        self.model.optimizer.zero_grad()
    ...
    def run_validation_pass(
        ...
        offline_known_frame_replay = bool(
            conf.validate_only
            and conf.post_processing.get("apply_known_frame_in_eval", False)
        )
        if reconstruction_validation:
            logger.info(
                "Validation uses known source-frame appearance for native "
        ...
        for val_iteration, batch_idx in enumerate(self.val_dataloader):
            ...
            with torch.cuda.nvtx.range(f"train.validation_step_{self.global_step}"):
                ...
                # Known-frame appearance is valid only when validation mirrors
                # training, or during an explicit offline reconstruction replay.
                if self.post_processing is not None:
                    outputs = apply_post_processing(
                        self.post_processing,
                        outputs,
