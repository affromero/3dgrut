...
def visibility_adaptive_topology_schedule(
    *,
    ...
    if reference_replay:
        regular_interval_enabled = step >= batch_size
        regular_prune = (
            regular_interval_enabled
            and prune_interval > 0
            and step % prune_interval == 0
        )
        batch_start = (step // batch_size) * batch_size
        densify_enabled = (
            total_iterations >= step + batch_size
        ...
        iterative_prune = (
            step + 1 >= batch_size
            and (step + 1) % batch_size == 0
            and total_iterations >= step + 1 + batch_size
        )
...
class VisibilityAdaptiveStrategy(BaseStrategy):
    ...
    def _post_backward(
        ...
        reference_replay = bool(
            self.conf.strategy.get("reference_replay", False)
        )
        ...
        accumulation_weight = projected_size
        if reference_replay:
            accumulation_weight = visibility_adaptive_projected_size_pixels(
                rendered_weight=rendered_weight,
                projected_extent=projected_extent,
                projected_conic_opacity=projected_conic_opacity,
                image_width=image_width,
        ...
        renderer_projected_gradient_pixels = None
        ...
        image_gradient_scale = camera_loss_multiplier / (
            fixed_image_loss_denominator(
                ...
                scale_by_image_resolution=bool(
                    self.conf.loss.get(
                        "fixed_image_loss_scale_by_resolution", True
                    )
                ),
        ...
        visibility_adaptive_preprocess_gradients(
            ...
            accumulation_weight_source=accumulation_weight[:, None],
    ...
    def _post_optimizer_step(
        ...
        changed = False
        if bool(self.conf.strategy.get("reference_replay", False)):
            if regular_prune:
                changed = self._prune() or changed
            if densify:
