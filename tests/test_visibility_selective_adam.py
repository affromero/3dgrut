...
def _optimizer(
    ...
    *,
    ...
    return VisibilitySelectiveAdam(
        groups,
        eps=eps,
...
def test_visibility_selective_adam_can_use_raw_moving_averages() -> None:
    """Reference replay can retain the archived uncorrected update rule."""
    parameters = _parameters(point_count=1)
    optimizer = _optimizer(parameters, bias_correction=False)
    parameters["positions"].grad = torch.ones_like(parameters["positions"])

    optimizer.step(torch.tensor((True,)))

    expected_update = 0.1 / (0.01**0.5) * POSITION_UPDATE_DECAY
    torch.testing.assert_close(
        parameters["positions"].detach(),
        torch.full((1, 3), -expected_update),
        atol=1.0e-6,
        rtol=0.0,
    )


def test_visibility_selective_adam_can_wrap_per_gaussian_age() -> None:
    """Reference replay preserves the archived uint16 age counter."""
    parameters = _parameters(point_count=1)
    optimizer = _optimizer(parameters, wrap_age=True)
    parameters["positions"].grad = torch.ones_like(parameters["positions"])
    optimizer.step(torch.tensor((True,)))
    optimizer.state[parameters["positions"]]["gaussian_steps"][0] = 65_535
