...
def test_native_rgb_denominator_can_ignore_image_scale() -> None:
    """Reference replay retains the archived full-image normalization."""
    rgb = torch.zeros((1, 10, 10, 3))
    mask = torch.ones((1, 10, 10, 1))
    ...
    denominator = fixed_image_loss_denominator(
        ...
        mask=mask,
        image_scale=0.05,
        scale_by_image_resolution=False,
    )

    torch.testing.assert_close(denominator, torch.tensor(300.0))
